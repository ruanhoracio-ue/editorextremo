# 📦 Como distribuir o EditorExtremo para os alunos (guia do instrutor)
...
Cada aluno instala o Docker Desktop uma vez e usa os atalhos `EditorExtremo`.
...
```bash
chmod +x EditorExtremo.command Parar-EditorExtremo.command
```
...
```bash
zip -r EditorExtremo-alunos.zip . \
  -x '*/node_modules/*' -x '*/.next/*' -x '*/venv/*' \
  -x '*/storage/*' -x '*/.git/*'
```

Isso gera `EditorExtremo-alunos.zip`. É esse arquivo que você manda para a turma
...
2. Descompacta o ZIP.
3. Dá dois cliques em **`EditorExtremo`** (`.bat` no Windows, `.command` no Mac).
...
- **Como atualizo o app depois?** Refaça os passos 1–3 e reenvie o ZIP. No
  computador do aluno, o atalho `EditorExtremo` já roda `docker compose up --build`,
